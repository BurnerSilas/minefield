...
void SerialControl::parseCommand(const char* cmd) {
    if      (strncmp(cmd, "DIST:", 5) == 0) handleDistortion(cmd + 5);
    else if (strncmp(cmd, "LIM:",  4) == 0) handleLimiter(cmd + 4);
    else if (strncmp(cmd, "LPF:",  4) == 0) handleLowPass(cmd + 4);
    else respond("ERR: unknown prefix. Use DIST:, LIM:, LPF:\n");
    if (strncmp(cmd, "DIST:BYPASS:", 12) == 0) handleBypass(m_distortion, cmd + 12);
    if (strncmp(cmd, "LIM:BYPASS:",  11) == 0) handleBypass(m_limiter,    cmd + 11);
    if (strncmp(cmd, "LPF:BYPASS:",  11) == 0) handleBypass(m_lowPass,    cmd + 11);
}
...
void SerialControl::handleDistortion(const char* sub) {
    if (strncmp(sub, "DRIVE:", 6) == 0) {
        float val = atof(sub + 6);
        m_distortion.setDrive(val);
        ESP_LOGI(TAG, "OK: DIST:DRIVE=%.2f", val);

    } else if (strncmp(sub, "LEVEL:", 6) == 0) {
        float val = atof(sub + 6);
        m_distortion.setLevel(val);
        ESP_LOGI(TAG, "OK: DIST:LEVEL=%.2f", val);

    } else {
        respond("ERR: unknown DIST command. Use DRIVE:, LEVEL:, BYPASS:\n");
    }
}
...
void SerialControl::handleLimiter(const char* sub) {
    if (strncmp(sub, "THRESH:", 7) == 0) {
        ...
        if      (strcmp(type, "HARD") == 0) m_limiter.setType(HARD);
        else if (strcmp(type, "SOFT") == 0) m_limiter.setType(SOFT);
        else respond("ERR: unknown type. Use HARD or SOFT\n");
